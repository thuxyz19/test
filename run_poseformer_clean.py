...
from common.model_group import *
...
if args.model_type == 'shift':
    model_pos_train = ShiftGroupTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                ...
                                num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None, drop_path_rate=args.droppath, M=9, shift=3)
elif args.model_type == 'dual':
    model_pos_train = ShiftDualTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.embed_ratio, depth=4,
            num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=args.droppath, M=9, shift=3)
elif args.model_type == 'ext':
    model_pos_train = ExtTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                      embed_dim_ratio=args.embed_ratio, depth=4,
                                      num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                      drop_path_rate=args.droppath, M=9, shift=3, seed=ref_3D)
elif args.model_type == 'ext-slight':
    if args.BN:
        model_pos_train = ExtSlightTransformerBN(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                               embed_dim_ratio=args.embed_ratio, depth=4,
                                               num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                               drop_path_rate=args.droppath, M=9, shift=3, seed=ref_3D,
                                               drop_rate=args.dropout)
    else:
        model_pos_train = ExtSlightTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                          embed_dim_ratio=args.embed_ratio, depth=4,
                                          num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                          drop_path_rate=args.droppath, M=9, shift=3, seed=ref_3D, drop_rate=args.dropout)
elif args.model_type == 'ext-group':
    model_pos_train = ExtGroupTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                          embed_dim_ratio=args.embed_ratio, depth=4,
                                          num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                          drop_path_rate=args.droppath, M=9, shift=3, seed=ref_3D, drop_rate=args.dropout)
else:
...
if args.model_type == 'shift':
    model_pos = ShiftGroupTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                            embed_dim_ratio=args.embed_ratio, depth=4,
                                            num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                            drop_path_rate=0.0, M=9, shift=3)
elif args.model_type == 'dual':
    model_pos = ShiftDualTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                           embed_dim_ratio=args.embed_ratio, depth=4,
                                           num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                           drop_path_rate=0.0, M=9, shift=3)
elif args.model_type == 'ext':
    model_pos = ExtTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                      embed_dim_ratio=args.embed_ratio, depth=4,
                                      num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                      drop_path_rate=0.0, M=9, shift=3, seed=ref_3D)
elif args.model_type == 'ext-slight':
    if args.BN:
        model_pos = ExtSlightTransformerBN(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                         embed_dim_ratio=args.embed_ratio, depth=4,
                                         num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                         drop_path_rate=0.0, M=9, shift=3, seed=ref_3D, drop_rate=args.dropout)
    else:
        model_pos = ExtSlightTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                          embed_dim_ratio=args.embed_ratio, depth=4,
                                          num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                          drop_path_rate=0.0, M=9, shift=3, seed=ref_3D, drop_rate=args.dropout)
elif args.model_type == 'ext-group':
    model_pos = ExtGroupTransformer(num_frame=receptive_field, num_joints=num_joints, in_chans=2,
                                          embed_dim_ratio=args.embed_ratio, depth=4,
                                          num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                                          drop_path_rate=0.0, M=9, shift=3, seed=ref_3D, drop_rate=args.dropout)
else:
...
if args.finetune:
    ...
    model_pos_train.load_state_dict(checkpoint, strict=True)
    model_pos.load_state_dict(checkpoint, strict=True)
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            if args.model_type == 'dual':
                ...
                ref_3D = torch.from_numpy(poses_act[0][start + receptive_field // 2:start + receptive_field // 2 + 1]).float().unsqueeze(
                    0).repeat(inputs_2d.shape[0], 1, 1, 1)
                ref_3D[:, :, 0] = 0.0
            ...
            if inputs_2d.shape[0] <= 2000:
                if args.model_type == 'dual':
                    predicted_3d_pos = model_pos(ref_3D, ref_2D, inputs_2d)
                    predicted_3d_pos_flip = model_pos(ref_3D, ref_2D, inputs_2d_flip)
                else:
                ...
                if args.model_type == 'dual':
                    for kk in range(inputs_2d.shape[0] // 2000 + 1):
                        predicted_3d_pos.append(
                            model_pos(ref_3D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...], ref_2D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...], inputs_2d[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]), ...]))
                        predicted_3d_pos_flip.append(
                            model_pos(ref_3D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...], ref_2D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...], inputs_2d_flip[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]), ...]))
                else:
                    for kk in range(inputs_2d.shape[0] // 2000 + 1):
                        predicted_3d_pos.append(
...
if not args.evaluate:
    ...
    # optimizer = optim.AdamW(model_pos_train.parameters(), eps=1e-8, betas=(0.9, 0.999),
    #                         lr=5e-4, weight_decay=0.05)

    # num_steps = int(20 * 6000)
    # warmup_steps = int(1 * 6000)
    ...
    # optimizer = optim.Adam(model_pos_train.parameters(), lr=lr, weight_decay=0.1)
    ...
    while epoch < args.epochs:
        ...
        if True:
            for cameras_train, batch_3d, batch_2d, batch_2d_gt in train_generator.next_epoch():
                ...
                # Predict 3D poses
                if args.model_type == 'dual':
                    ref_2D = torch.cat([inputs_2d[1:, ...], inputs_2d[:1, ...]], 0)
                    ref_3D = torch.cat([inputs_3d[1:, ...], inputs_3d[:1, ...]], 0)
                    predicted_3d_pos = model_pos_train(ref_3D, ref_2D, inputs_2d)
                elif args.model_type == 'ext' or 'ext-slight' or 'ext-group':
                ...
                optimizer.step()
                # lr_scheduler.step_update(total_count)
                    ...
                    if inputs_2d.shape[0] <= 2000:
                        ...
                        if args.model_type == 'dual':
                            for kk in range(inputs_2d.shape[0] // 2000 + 1):
                                predicted_3d_pos.append(
                                    model_pos(ref_3D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...], ref_2D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...], inputs_2d[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]), ...]))
                                predicted_3d_pos_flip.append(model_pos(ref_3D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...], ref_2D[kk * 2000: min(kk * 2000 + 2000, inputs_2d.shape[0]),...],
