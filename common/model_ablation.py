...
class GroupAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        for i in range(len(self.groups)):
            ...
            x_i, attn_i = self.attn[i](x_i, mask)
            x_.append(x_i.view(*x_i.shape[:2], len(self.groups[i]), -1))
            attn_.append(attn_i)
        x = torch.cat(x_, 2)
        attn = torch.stack(attn_, 0)
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None):
        if self.attn_type is RelativeAttention:
            x_tmp, attn = self.attn(self.norm1(x), mask)
...
class ShiftGroupBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(self.shift_attn1(self.norm2(x), mask)[0])
        ...
        x = x.view(b, -1, self.M, c).view(-1, self.M, c)
        mask = mask.repeat(b, 1, 1)
        x_tmp, attn = self.shift_attn2(self.norm3(x), mask)
        x = x + self.drop_path(x_tmp)
...
class ShiftBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.norm1 = norm_layer(dim)
        self.attn = Block(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, seq=M, attn_type=RelativeAttention)
        self.norm2 = norm_layer(dim)
        self.shift_attn1 = Block(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, seq=M, attn_type=RelativeAttention)
        self.norm3 = norm_layer(dim)
        self.shift_attn2 = Block(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, seq=M, attn_type=RelativeAttention)
...
class NoExt(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        self.ext_blocks = nn.ModuleList([
            GroupBlock(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                       drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i-2], norm_layer=norm_layer, downsample=down_sample[i])
    ...
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        self.ext_blocks = nn.ModuleList([
            Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                       drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i-2], norm_layer=norm_layer, attn_type=RelativeAttention)
            for i in range(2, depth)
        ])
...
class ExtSlight(nn.Module):
    ...
    def forward_features(self, x, ref_3D=None):
        ...
        x = x.view(b, 1, p, -1)
        # x = x.view(b, -1)
        return x, self_attn, mean_attn / 2.0
    ...
    def forward(self, x, ref_3D=None):
        ...
        b, _, _, p = x.shape
        ### now x is [batch_size, 2 channels, receptive frames, joint_num], following image data
        x, self_attn, ext_attn = self.forward_features(x, ref_3D)
        self_attn = self_attn.transpose(0, 1).contiguous()
        ext_attn = ext_attn.transpose(0, 1).contiguous()
        # self_attn: (B, 5, H, 9, 9)
        # ext_attn: (B, 5, H, 9, 1024)
        x = self.Spatial_forward_features(x)
        x = self.head(x)
        x = x.view(b, p, -1)
        if self.refine is not None:
