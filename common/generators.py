...
class ChunkedDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if pad_left_2d != 0 or pad_right_2d != 0:
            batch_2d = np.pad(seq_2d[low_2d:high_2d], ((pad_left_2d, pad_right_2d), (0, 0), (0, 0)), 'edge')
        else:
        ...
        if self.poses_3d is not None:
            ...
            if pad_left_3d != 0 or pad_right_3d != 0:
                batch_3d = np.pad(seq_3d[low_3d:high_3d], ((pad_left_3d, pad_right_3d), (0, 0), (0, 0)), 'edge')
            else:
        ...
        if self.poses_3d is None and self.cameras is None:
            return None, None, batch_2d
        elif self.poses_3d is not None and self.cameras is None:
            return None, batch_3d, batch_2d
        elif self.poses_3d is None:
            return batch_cam, None, batch_2d
        else:
...
class ChunkedGenerator:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d, poses_2d_gt,
        ...
        if self.inter:
            self.batch_3d_inter = np.empty((batch_size, chunk_length + 2*pad, poses_3d[0].shape[-2], poses_3d[0].shape[-1]))
        self.batch_2d = np.empty((batch_size, chunk_length + 2*pad, poses_2d[0].shape[-2], poses_2d[0].shape[-1]))
        self.batch_2d_gt = np.empty((batch_size, chunk_length + 2 * pad, poses_2d_gt[0].shape[-2], poses_2d_gt[0].shape[-1]))
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    ...
                    if pad_left_2d != 0 or pad_right_2d != 0:
                        ...
                        self.batch_2d_gt[i] = np.pad(seq_2d_gt[low_2d:high_2d], ((pad_left_2d, pad_right_2d), (0, 0), (0, 0)),
                                                  'edge')
                    else:
                    ...
                    if flip:
                        ...
                        self.batch_2d_gt[i, :, self.kps_left + self.kps_right] = self.batch_2d_gt[i, :,
                    ...
                    if not self.inter:
                        ...
                        if self.poses_3d is not None:
                            ...
                            start_2d = start_3d - self.pad - self.causal_shift
                            end_2d = end_3d + self.pad - self.causal_shift
                            low_3d = max(start_2d, 0)
                            high_3d = min(end_2d, seq_3d.shape[0])
                            pad_left_3d = low_3d - start_2d
                            pad_right_3d = end_2d - high_3d
                            if pad_left_3d != 0 or pad_right_3d != 0:
                                self.batch_3d_inter[i] = np.pad(seq_3d[low_3d:high_3d],
                                                          ((pad_left_3d, pad_right_3d), (0, 0), (0, 0)), 'edge')
                            else:
                                self.batch_3d_inter[i] = seq_3d[low_3d:high_3d]

                            if flip:
                                # Flip 3D joints
                                self.batch_3d_inter[i, :, :, 0] *= -1
                                self.batch_3d_inter[i, :, self.joints_left + self.joints_right] = \
                                    self.batch_3d_inter[i, :, self.joints_right + self.joints_left]
                ...
                if self.poses_3d is None and self.cameras is None:
                    yield None, None, self.batch_2d[:len(chunks)], self.batch_2d_gt[:len(chunks)]
                elif self.poses_3d is not None and self.cameras is None:
                    yield None, self.batch_3d[:len(chunks)], self.batch_2d[:len(chunks)], self.batch_2d_gt[:len(chunks)]
                elif self.poses_3d is None:
                    yield self.batch_cam[:len(chunks)], None, self.batch_2d[:len(chunks)], self.batch_2d_gt[:len(chunks)]
                elif self.inter:
                    yield self.batch_cam[:len(chunks)], self.batch_3d_inter[:len(chunks)], self.batch_2d[
                                                                                     :len(chunks)], self.batch_2d_gt[
                                                                                                    :len(chunks)]
                else:
